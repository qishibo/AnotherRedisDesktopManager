const ru = {
  message: {
    ...
    update_downloaded:
      'Обновление завершено, перезапустите приложение, пожалуйста.\
    ...
    mac_not_support_auto_update:
      'Mac не поддерживает автоматическое обновление, <a href="https://github.com/qishibo/AnotherRedisDesktopManager/releases">скачайте</a> и переустановите приложение вручную,\
    ...
    font_faq:
      '1. Можно установить несколько шрифтов.<br>\
    ...
    private_key_faq:
      'В настоящее время поддерживаются закрытые ключи формата RSA, начинающиеся с <pre>-----BEGIN RSA PRIVATE KEY-----</pre>\
    ...
    confirm_modify_unvisible_content:
      'Контент содержит невидимые символы, вы можете смело редактировать его в «Hex View». Если продолжение редактирования в «Text View» может вызвать ошибки кодирования, обязательно продолжить?',
    ...
    sentinel_faq:
      'Вы можете выбрать один из нескольких дозорных. Пожалуйста, заполните конфигурацию дозорных для адреса, порта и пароля. Пароль узла Redis - это пароль главного узла, который контролируется дозорным.',
    ...
    load_all_keys_tip:
      'Загрузите все ключи одновременно. Если количество ключей слишком велико, клиент может застрять. Пожалуйста, используйте его правильно',
    tree_node_overflow:
      'Слишком много клавиш или папок, оставьте только {num} для отображения. Если вашего ключа здесь нет, рекомендуется использовать нечеткий поиск, или Настройка разделителя для разделения ключей в папку',
    ...
    before_connect: 'Перед подключением (команда)',
    before_connect_error: 'Перед ошибкой подключения (команды): {message}',
