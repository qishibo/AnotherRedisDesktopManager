...
> 当默认可视化方式不满足需求时，可以自定义脚本来格式化你的内容。
> 方式：可视化列表下拉到底部，点击"自定义->新增"，然后参考下面说明。
> **注意**脚本需要通过`print` `console.log` `echo`等输出格式化好的内容，可以是任意字符串或者JSON字符串


| 配置项 | 参数说明 |
| ------ | ------ |
| `Name` | 自定义名称 |
| `Command` | 可执行命令，如`xxx.py` `xxx.js` `xxx.class`等。该文件需要具有可执行的`x`权限，可以通过形如`./xxx.py`方式执行 |
...
配置样例如下：
> 最终执行的命令如: `./home/qii/pickle_decoder.py {HEX}`, 脚本中可以使用`argv[1]`接收参数
...
当脚本文件无执行权限`x`时，可以使用如下方法，将脚本文件也做为参数传入：
